...
{
    public partial class BedSideView3 : Form
    {
        public static BedSideView3 bed3singleton = new BedSideView3();
        ...
        public void UpdateColor(object sender, EventArgs e)
        {
            ...
            int.TryParse(CentralStation.centralsingleton.tbdi3.Text, out ditext); // Diastolic Value
            int.TryParse(CentralStation.centralsingleton.tbsy3.Text, out sytext); // Systolic Value
            int.TryParse(CentralStation.centralsingleton.tbpr3.Text, out prtext); // Pulse Value
            int.TryParse(CentralStation.centralsingleton.tbbr3.Text, out brtext); // Breathing Value
            int.TryParse(CentralStation.centralsingleton.tbtp3.Text, out tptext); // Temperature Value

            #region Textbox Color
            // Diastolic TextBox Color
            if (ditext < SocketSelection.diMin3 || ditext > SocketSelection.diMax3)
            {
                CentralStation.centralsingleton.tbdi3.BackColor = Color.Red;
            }
            else if (ditext > SocketSelection.diMin3 && ditext < SocketSelection.diMax3)
            {
                CentralStation.centralsingleton.tbdi3.BackColor = Color.FromArgb(105, 105, 105);
            }

            // Systolic TextBox Color
            if (sytext < SocketSelection.syMin3 || sytext > SocketSelection.syMax3)
            {
                CentralStation.centralsingleton.tbsy3.BackColor = Color.Red;
            }
            else if (sytext > SocketSelection.syMin3 && sytext < SocketSelection.syMax3)
            {
                CentralStation.centralsingleton.tbsy3.BackColor = Color.FromArgb(105, 105, 105);
            }

            // Pulse Textbox Color
            if (prtext < SocketSelection.prMin3 || prtext > SocketSelection.prMax3)
            {
                CentralStation.centralsingleton.tbpr3.BackColor = Color.Red;
            }
            else if (prtext > SocketSelection.prMin3 && prtext < SocketSelection.prMax3)
            {
                CentralStation.centralsingleton.tbpr3.BackColor = Color.FromArgb(105, 105, 105);
            }

            // Breathing Textbox Color
            if (brtext < SocketSelection.brMin3 || brtext > SocketSelection.brMax3)
            {
                CentralStation.centralsingleton.tbbr3.BackColor = Color.Red;
            }
            else if (brtext > SocketSelection.brMin3 && brtext < SocketSelection.brMax3)
            {
                CentralStation.centralsingleton.tbbr3.BackColor = Color.FromArgb(105, 105, 105);
            }

            // Temperature Textbox Color
            if (tptext < SocketSelection.tpMin3 || tptext < SocketSelection.tpMax3)
            {
                CentralStation.centralsingleton.tbtp3.BackColor = Color.Red;
            }
            else if (tptext > SocketSelection.tpMin3 && tptext > SocketSelection.tpMax3)
            {
                CentralStation.centralsingleton.tbtp3.BackColor = Color.FromArgb(105, 105, 105);
            }
            #endregion
        ...
        public void UpdateCentralStation(object sender, EventArgs e)
        {
            if (bloodActive == true)
            {
                CentralStation.centralsingleton.tbdi3.Text = DiastolicValueRandom();
                CentralStation.centralsingleton.tbsy3.Text = SystolicValueRandom();
            }
            else
            {
                CentralStation.centralsingleton.tbdi3.Text = "";
                CentralStation.centralsingleton.tbsy3.Text = "";
            }

            if (breathingActive == true)
            {
                CentralStation.centralsingleton.tbbr3.Text = BreathingValueRandom();
            }
            else { CentralStation.centralsingleton.tbbr3.Text = ""; }

            if (pulseActive == true)
            {
                CentralStation.centralsingleton.tbpr3.Text = PulseValueRandom();
            }
            else { CentralStation.centralsingleton.tbpr3.Text = ""; }

            if (tempActive == true)
            {
                CentralStation.centralsingleton.tbtp3.Text = TemperatureValueRandom();
            }
            else { CentralStation.centralsingleton.tbtp3.Text = ""; }
        ...
        private void InsertButton1_Click(object sender, EventArgs e)
        {
            if (moduleState1 == 0)
            {
                ...
                SocketSelection.socketsingleton.BedLabel.Text = "Bed 3";
        ...
        public static string SystolicValueRandom() // Returns syValue
        {
            ...
            if (randomizerSy <= 5)
            {
                syValue = SocketConfiguration.randomizer.Next(SocketSelection.syMin3 - 20, SocketSelection.syMin3 - 1);
            }
            else if ((randomizerSy > 5) && (randomizerSy < 95))
            {
                syValue = SocketConfiguration.randomizer.Next(SocketSelection.syMin3, SocketSelection.syMax3);
            }
            else if (randomizerSy >= 95)
            {
                syValue = SocketConfiguration.randomizer.Next(SocketSelection.syMax3 + 1, SocketSelection.syMax3 + 20);
            }
        ...
        public static string DiastolicValueRandom() // Returns diValue
        {
            ...
            if (randomizerDi <= 5)
            {
                diValue = SocketConfiguration.randomizer.Next(SocketSelection.diMin3 - 20, SocketSelection.diMin3 - 1);
            }
            else if ((randomizerDi > 5) && (randomizerDi < 95))
            {
                diValue = SocketConfiguration.randomizer.Next(SocketSelection.diMin3, SocketSelection.diMax3);
            }
            else if (randomizerDi >= 95)
            {
                diValue = SocketConfiguration.randomizer.Next(SocketSelection.diMin3 + 1, SocketSelection.diMax3 + 20);
            }
        ...
        public static string PulseValueRandom()
        {
            ...
            if (randomizerPR <= 5)
            {
                prValue = SocketConfiguration.randomizer.Next(SocketSelection.prMin3 - 20, SocketSelection.prMin3 - 1);
            }
            else if ((randomizerPR > 5) && (randomizerPR < 95))
            {
                prValue = SocketConfiguration.randomizer.Next(SocketSelection.prMin3, SocketSelection.prMax3);
            }
            else if (randomizerPR >= 95)
            {
                prValue = SocketConfiguration.randomizer.Next(SocketSelection.prMax3 + 1, SocketSelection.prMax3 + 20);
            }
        ...
        public static string BreathingValueRandom()
        {
            ...
            if (randomizerBR <= 5)
            {
                brValue = SocketConfiguration.randomizer.Next(SocketSelection.brMin3 - 5, SocketSelection.brMin3 - 1);
            }
            else if ((randomizerBR > 5) && (randomizerBR < 95))
            {
                brValue = SocketConfiguration.randomizer.Next(SocketSelection.brMin3, SocketSelection.brMax3);
            }
            else if (randomizerBR >= 95)
            {
                brValue = SocketConfiguration.randomizer.Next(SocketSelection.brMax3 + 1, SocketSelection.brMax3 + 50);
            }
        ...
        public static string TemperatureValueRandom()
        {
            ...
            {
                tempValue = SocketConfiguration.randomizer.Next(SocketSelection.tpMin3 - 25, SocketSelection.tpMin3 - 1);
            }
            else if ((randomizerTemp > 25) && (randomizerTemp < 75))
            {
                tempValue = SocketConfiguration.randomizer.Next(SocketSelection.tpMin3, SocketSelection.tpMax3);
            }
            else if (randomizerTemp >= 75)
            {
                tempValue = SocketConfiguration.randomizer.Next(SocketSelection.tpMax3 + 1, SocketSelection.tpMax3 + 45);
            }
